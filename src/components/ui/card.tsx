...
export default function SlideCard({ slide }: SlideCardProps) {
  ...
  return (
    <div className="group relative w-full h-[25rem] lg:h-[30rem] overflow-hidden rounded-3xl mx-auto" onClick={toggleVisibility}>
      ...
      <div className="absolute inset-0 flex flex-col justify-end bg-black/40 p-6 pointer-events-none">
      ...
      <div className={`absolute inset-0 flex flex-col bg-black/70 p-6 rounded-3xl transition-opacity duration-300 ${ visible ? "opacity-100" : "opacity-0" } md:opacity-0 md:group-hover:opacity-100`}>
        <div className="absolute inset-0 flex flex-col px-6 py-12 md:py-24 gap-4 pointer-events-none">
          <Quote className="w-10 h-10 text-white" />
          <h2 className="text-lg md:text-xl font-montserrat text-white font-light drop-shadow-lg px-8">
            {slide.text}
          </h2>
        </div>
